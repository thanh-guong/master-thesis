...
def rml201610a_VTCNN2_v2_double_input(A_input_shape, B_input_shape, dropout_rate=0.5, classes=11):
    # input A
    A_input = keras.Input(shape=A_input_shape)
    A_reshape = Reshape(A_input_shape + [1])(A_input)

    A_conv_1 = Convolution2D(256, 1, 3, padding="same", activation="relu")(A_reshape)
    A_dr_1 = Dropout(dropout_rate)(A_conv_1)
    A_conv_2 = Convolution2D(80, 2, 3, padding="same", activation="relu")(A_dr_1)
    A_dr_2 = Dropout(dropout_rate)(A_conv_2)

    # input B
    B_input = keras.Input(shape=B_input_shape)
    B_reshape = Reshape(B_input_shape + [1])(B_input)

    B_conv_1 = Convolution2D(256, 1, 3, padding="same", activation="relu")(B_reshape)
    B_dr_1 = Dropout(dropout_rate)(B_conv_1)
    B_conv_2 = Convolution2D(80, 2, 3, padding="same", activation="relu")(B_dr_1)
    B_dr_2 = Dropout(dropout_rate)(B_conv_2)

    # A and B chains concatenation
    x = concatenate([A_dr_2, B_dr_2])

    out_flatten = Flatten()(x)
    fc1 = Dense(256, activation="relu")(out_flatten)
    output = Dense(classes, name="output", activation="softmax")(fc1)
...
def baseline(input_shape, classes=11):
...
def custom_baseline(input_shape, classes=11):
    iq_in = keras.Input(shape=input_shape, name="IQ")

    conv_1 = Convolution1D(128, 7, padding="same", activation="relu")(iq_in)
    conv_2 = Convolution1D(128, 5, padding="same", activation="relu")(conv_1)
...
def custom_baseline_v2(input_shape, classes=11):
    iq_in = keras.Input(shape=input_shape, name="IQ")

    conv_1 = Convolution1D(36, 2, padding="same", activation="relu")(iq_in)
    conv_2 = Convolution1D(18, 4, padding="same", activation="relu")(conv_1)
    max_pool = MaxPooling1D(padding='same')(conv_2)
